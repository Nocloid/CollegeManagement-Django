...
def student_feedback_message(request):
    if request.method != 'POST':
        retroalimentacións = FeedbackStudent.objects.all()
        context = {
            'retroalimentacións': retroalimentacións,
        ...
        try:
            retroalimentación = get_object_or_404(FeedbackStudent, id=feedback_id)
            reply = request.POST.get('reply')
            retroalimentación.reply = reply
            retroalimentación.save()
            return HttpResponse(True)
        except Exception as e:
            return HttpResponse(False)
...
def staff_feedback_message(request):
    if request.method != 'POST':
        retroalimentaciones = FeedbackStaff.objects.all()
        context = {
        ...
        feedback_id = request.POST.get('id')
        try:
            retroalimentación = get_object_or_404(FeedbackStaff, id=feedback_id)
            reply = request.POST.get('reply')
            retroalimentación.reply = reply
            retroalimentación.save()
            return HttpResponse(True)
